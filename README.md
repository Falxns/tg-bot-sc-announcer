...
- `/editrolepanel channel:<channel> message_id:<snowflake> [role1…role6] [label1…label6] [single_role] [embed_*] [image]` — edit an existing **role panel** message (the one with role buttons); omit roles to keep current buttons; set **`role1`** (and optional others) to replace buttons; modal edits body text (max **2000** chars on that message)
- `/editlinkpanel channel:<channel> message_id:<snowflake> [url1…url5] [label1…label5] [embed_*] [image]` — edit an existing **link button** message; omit URLs to keep current buttons; set **`url1`** (and optional others) to replace links; same embed/file/modal rules as `/edit`
